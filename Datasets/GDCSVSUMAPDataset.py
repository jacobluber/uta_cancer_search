...
class GDCSVSDataset(Dataset):
    ...
    def Prepare(self):
        ...
        elif "," in self.cancer_type:
            types = self.cancer_type.split(",")
            types = [t.strip() for t in types]

            meta = gdc_meta[gdc_meta["primary_site"].isin(types)]

            num_images_by_type_dict = meta.groupby('primary_site').count()['id'].to_dict()

            for key, value in num_images_by_type_dict.items():
                num_images_by_type_dict[key] = int(np.ceil(self.ratio_per_type * value))

            metadata = meta.groupby('primary_site', group_keys=True).apply(
                lambda x: x.sample(n=num_images_by_type_dict[x.iloc[0].primary_site])
            ).reset_index(drop=True).copy(deep=True)
        ...
        if self.dataset_type == "predict":
            if "," in self.cancer_type:
                npc_predict = self._calculate_coords(predict_image_filenames, 'predict')
            else:
    ...
    def _calculate_coords(self, filenames, split_type):
        """
        `split_type` can be 'train', 'val', 'test', or 'predict'
        """
